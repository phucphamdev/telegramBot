    ...
    <script type="text/javascript">
        ...
        (function ($) {
            ...
            $('#recharge').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: '{{ route('recharge.datatables') }}',
                columns: [
            ...
            setTimeout(function () {
                $.ajax({
                    type: 'get',
                    dataType: 'json',
                    url: "{{ route('recharge.datatables') }}",
                    contentType: "application/json; charset=utf-8",
                    traditional: true,
                    success: function (data) {
                        $.each(data.data, function (index, value) {
                            if (value.status == 'pending') {
                                let name_time = "time_" + value.id;
                                let time = $("input[name='" + name_time + "']").val();
                                console.log(time);

                                if (!isNaN(time)) {
                                    let timeresult = "timeresult_" + value.id;
                                    ...
                                    var x = setInterval(function () {
                                        ...
                                        document.getElementById(timeresult).innerHTML = '<div class="badge py-3 px-4 fs-7 badge-light-warning" >' + minutes + ' m ' + seconds + ' s  </div>';
                                        if (distance < 0) {
                                            clearInterval(x);
                                            document.getElementById(timeresult).innerHTML = document.getElementById(timeresult).innerHTML = '<div class="badge py-3 px-4 fs-7 badge-light-warning" >EXPIRED</div>';
                                        }
                                    }, 1000);
                                } else {
                                    console.log('time isNaN');
                                }
                            }
                        });
                    }
