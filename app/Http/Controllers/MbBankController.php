...
//		protected $captchaKey = 'c668ae146516be44009aac0252761fca';
		...
		function doLogin()
		{
			...
			if (isset($curl['result']['ok']) && $curl['result']['ok'] == false) {
				...
				System::updateOrCreate(['id' => 999], $data);
		...
		{
			$this->loading();
//		$a = new MbBankAController();
//		$a->loading();
			
			$object = System::where('id', 999)->first();
			$history = $this->getHistory($object->mbbank_username, $object->mbbank_account_no, $object->mbbank_sessionId, $object->mbbank_imei, 3);
			...
			if (isset($history['transactionHistoryList'])) {
				$this->processTransactionHistoryList($history['transactionHistoryList']);
			} else {
				$this->doLoginAndLoading();
			}
		...
		public function isHistoryOk($history)
		{
			return isset($history['result']['ok']);
		}
		
		public function processTransactionHistory($history)
		{
			$this->sendTransactionCountMessage($history);
			$this->processTransactionHistoryList($history['transactionHistoryList']);
		}
		
		public function sendTransactionCountMessage($history)
		{
			Telegram::sendMessage([
			  'chat_id' => "1281282845",
			  'parse_mode' => 'HTML',
			  'text' => 'MBBank - transactionHistoryList  : ' . count($history['transactionHistoryList'])
			]);
		}
		
		public function processTransactionHistoryList($transactionHistoryList)
		{
			foreach ($transactionHistoryList as $item) {
				if (!$this->transactionExists($item['refNo'])) {
					$this->createTransaction($item);
				}
			}
		}
		
		public function doLoginAndLoading()
		{
			$this->doLogin();
			$this->loading();
		}
		
		public function transactionExists($refNo)
		{
			return transactionMbBankHistoryList::where('refNo', $refNo)->exists();
		}
