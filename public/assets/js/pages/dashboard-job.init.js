...
// recomended-jobs2
if (document.getElementById("recomended-jobs2")){
    var jobListAll2 = new gridjs.Grid({
        columns: [{
            name: 'Position',
            width: '150px',
        }, {
        ...
        data: jobListAllData2,
    }).render(document.getElementById("recomended-jobs2"));

    // Search product list
    var searchResultList2 = document.getElementById("searchResultList2");
    searchResultList2.addEventListener("keyup", function () {
        var inputVal = searchResultList2.value.toLowerCase();
        function filterItems(arr, query) {
            return arr.filter(function (el) {
                return el[0].toLowerCase().indexOf(query.toLowerCase()) !== -1 || el[1].toLowerCase().indexOf(query.toLowerCase()) !== -1
            })
        ...
        var filterData = filterItems(jobListAllData2, inputVal);

        jobListAll2.updateConfig({
            data: filterData
        }).forceRender();
    });
}
